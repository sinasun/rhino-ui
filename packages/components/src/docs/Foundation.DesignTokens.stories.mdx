...
import { color } from '@rhinolabs/design-tokens/build/json/variables-color.json';
import { size } from '@rhinolabs/design-tokens/build/js/variables-size';
import { asset } from '@rhinolabs/design-tokens/build/js/variables-asset';
...
[Rhinolabs Design Tokens](https://github.com/rhinolabs/rhinolabs-design-tokens) are available as an [npm package](https://www.npmjs.com/package/@rhinolabs/design-tokens) so that you can create a Rhinolabs unified product experience for your platform of choice.
...
Rhinolabs components follow a "mobile-first" approach, where styles are set with mobile devices considered first. Then styling adjustments are made as each minimum viewport width &mdash; the breakpoint &mdash; is reached.
